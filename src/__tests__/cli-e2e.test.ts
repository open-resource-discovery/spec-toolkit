...
describe("CLI End-to-End Tests", () => {
  ...
  // warned about (stderr is not required to be empty), and the run succeeds.
  test("test 5: tolerant run with a non-convention-clean schema still generates docs", async () => {
    ...
    const cliArguments = [cliScriptPath, "-c", configFile];
    const staleTypesFile = "src/__tests__/generated/test5/my-tolerant-spec-v1/types/my-tolerant-spec.ts";
    fs.mkdirSync("src/__tests__/generated/test5/my-tolerant-spec-v1/types", { recursive: true });
    fs.writeFileSync(staleTypesFile, "// stale generated types");
    ...
    // json-schema-to-typescript v16 supports this conditional schema, so stale
    // output is replaced with fresh type declarations rather than preserved.
    expect(fs.readFileSync(staleTypesFile, "utf8")).toContain("export");
    ...
    const generatedSchema = JSON.parse(
      fs.readFileSync("src/__tests__/generated/test5/my-tolerant-spec-v1/schemas/my-tolerant-spec.schema.json", "utf8"),
    ) as { definitions: { Config: { type?: string } } };
    expect(generatedSchema.definitions.Config.type).toBeUndefined();
  });

  test("test 6: strict mode is the default and rejects a non-convention-clean schema", async () => {
